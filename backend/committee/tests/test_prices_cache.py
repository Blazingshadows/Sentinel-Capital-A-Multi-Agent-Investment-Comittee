...
def test_fetch_ohlcv_accumulates_across_calls_instead_of_overwriting(tmp_path, monkeypatch):
    """yfinance's period cap is a per-call window, not a ceiling on the
    cache -- repeated calls over time should grow the effective history
    rather than each call discarding what came before."""
    ...
    monkeypatch.setattr(prices.yf, "download", lambda *a, **k: _fake_ohlcv(first_days, first_closes))
    ...
    monkeypatch.setattr(prices.yf, "download", lambda *a, **k: _fake_ohlcv(second_days, second_closes))
